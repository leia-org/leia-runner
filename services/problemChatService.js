...
const SYSTEM_PROMPT = [
  ...
  '- list_behaviours() / use_behaviour(id): list the instructor\'s EXISTING behaviours and REUSE one by id instead of creating a new one. Likewise list_personas() / use_persona(id) for personas.',
  ...
  'Guidance:',
  '- When the user asks for an activity/LEIA (or attaches a PDF), assemble the WHOLE LEIA: a problem, a behaviour and a persona that fit together. For the behaviour and persona, FIRST call list_behaviours / list_personas; if a suitable one already exists, REUSE it with use_behaviour(id) / use_persona(id); only create a new one (apply_behaviour / apply_persona) when none fits. Write the problem with apply_problem. If the user only asks about the problem, you may write just the problem.',
